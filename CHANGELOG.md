...
- **Owned shutdown drainage.** Shutdown now rejects new background work, drains the current album without advancing queued downloads, interrupts and records active syncs, cooperatively stops scans after any in-flight thread mutation, waits for progress events, and only then closes current SDK clients.
